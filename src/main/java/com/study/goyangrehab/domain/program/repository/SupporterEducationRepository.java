package com.study.goyangrehab.domain.program.repository;

import com.study.goyangrehab.domain.program.entity.SupporterEducation;
import com.study.goyangrehab.enums.ProgramStatus;
import org.springframework.data.jpa.repository.JpaRepository;

import java.util.List;
import java.util.Optional;

public interface SupporterEducationRepository extends JpaRepository<SupporterEducation, Long> {
    Optional<List<SupporterEducation>> findAllByStatusIs(ProgramStatus status);


}
